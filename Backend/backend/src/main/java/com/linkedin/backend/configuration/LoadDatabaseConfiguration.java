...
public class LoadDatabaseConfiguration {
    ...
    @Bean
    public CommandLineRunner initDatabase(UserRepository userRepository, LearningPlanRepository learningPlanRepository, PostRepository postRepository, ConnectionRepository connectionRepository) {
        return args -> {
            List<User> users = createUsers(userRepository);
            createConnections(connectionRepository, users);
            createLearningPlans(learningPlanRepository,users);
    ...
    private void createLearningPlans(LearningPlanRepository learningPlanRepository,List<User>users) {
        List<LearningPlan> plans = new ArrayList<>();

        for (int i = 0; i < 5; i++) {
            User creator = users.get(i % users.size());
            LearningPlan plan = new LearningPlan();
            plan.setTitle("Plan " + (i + 1));
            plan.setDescription("This is a description for Plan " + (i + 1));


            List<Milestone> milestones = new ArrayList<>();
            for (int j = 1; j <= 3; j++) {
                Milestone milestone = new Milestone();
                milestone.setTitle("Milestone " + j + " for Plan " + (i + 1));
                milestone.setDescription("Details about milestone " + j);
                milestone.setLearningPlan(plan); // establish bidirectional relationship

                List<TaskStat> tasks = new ArrayList<>();
                for (int k = 1; k <= 2; k++) {
                    TaskStat task = new TaskStat();
                    task.setTitle("Task " + k + " of Milestone " + j);
                    task.setStatus("PENDING");
                    task.setMilestone(milestone); // assign to milestone
                    tasks.add(task);
                }

                milestone.setTasks(tasks); // assuming setTasks() exists
                milestones.add(milestone);
            }

            plan.setMilestones(milestones); // assuming setMilestones() exists
            plans.add(plan);
        }

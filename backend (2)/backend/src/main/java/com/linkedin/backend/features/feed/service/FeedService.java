...
public class FeedService {
    ...
    public Post likePost(Long postId, Long userId) {
        logger.info("Liking post {} by user {}", postId, userId);
        ...
        boolean alreadyLiked = post.getLikes().stream()
                .anyMatch(likeUser -> likeUser.getId().equals(userId));

        if (alreadyLiked) {
            logger.info("User {} already liked post {}. Removing like.", userId, postId);
            post.getLikes().removeIf(likeUser -> likeUser.getId().equals(userId));
        } else {
            logger.info("User {} liking post {} for the first time", userId, postId);
            post.getLikes().add(user);
            notificationService.sendLikeNotification(user, post.getAuthor(), post.getId());
        }

        Post savedPost = postRepository.save(post);
        logger.info("Post {} saved with {} likes", postId, savedPost.getLikes().size());
